...
// the in-process /health endpoint and the docker daemon's container-health
// state. These tests pin the contract that holds the boot-non-fatal promise
// together: when wavehouse is boot-degraded, /health returns 503 and the
...
func TestRunHealthCheck_Success(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// The probe targets /health specifically — pin that contract here
		// so a refactor that points the probe at /ready (which requires
...
func TestRunHealthCheck_BootDegraded503(t *testing.T) {
	...
	// in-process /health returns 503 with a diagnostic body, the probe
