...
3. **Schema-driven ingest**: `POST /v1/ingest?table={table}` accepts a flat JSON object, a JSON array of objects, or an NDJSON batch. The table name comes from the `table` query parameter. The body is validated against the discovered schema (unknown fields rejected, types checked, nullable constraints enforced). No envelope — just data. The **format is auto-detected from the body — the `Content-Type` is only a hint** (the first non-whitespace byte decides; an explicit `application/x-ndjson` selects line-framing unless the body starts with `[`). A batch (array/NDJSON) fans into one independently-validated/published message per record and returns a per-record `{total,succeeded,failed,duplicates,results:[{index, ok|duplicate|error}]}` summary, so one malformed/rejected record never fails the rest of the batch; a lone object keeps its `{"ok":true}` / `{"duplicate":true}` response. Whole-request conditions — table-level `insert` denial, a JSON-array syntax error / oversized NDJSON line (`400`), the 16 MiB body cap (`413`, parity with `/v1/admin/query`), backpressure (`503`), publish/dedup-backend errors (`500`) — still abort non-200. Formats sit behind a pull-based `recordReader` seam (object / array / line readers + a body sniffer) in `internal/api/record_reader.go` so CSV and response-streaming slot in later; the single and batch paths in `internal/api/ingest.go` share the `processRecord` helper. Bounded-concurrency uploading of large batches is deferred to #196.
