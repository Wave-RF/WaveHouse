...
Documentation *prose* — accuracy against the code, runnable examples, clarity, completeness — is reviewed by the **`docs-reviewer`** subagent, not the code-focused `pre-push-reviewer`. Run it via **`/docs-review`** (no arg → the branch's docs diff vs `main`; a path/glob → those files; `all` → the whole site). The canonical rubric is `.github/prompts/docs-review.md`. It runs in fresh context and is **advisory**: it surfaces `[MUST]`/`[SHOULD]`/`[MAY]` findings, never edits docs, posts no PR comments, and writes no marker (the SubagentStop hook is keyed to `pre-push-reviewer`, so docs review cannot gate a push). It complements the deterministic prose tools — misspell, markdownlint, starlight-links-validator — reviewing only what they can't.

**Soft-gate before shipping docs:** when a PR changes docs prose, `pre-push-reviewer` raises a `[SHOULD]` recommending `/docs-review` be run before `ship_it` (which, under the zero-findings rubric, forces `iterate`). It's a nudge, not a hard gate — but a docs-prose change shouldn't merge un-reviewed. The whole dev team runs Claude Code and this command is tracked in-repo, so everyone can run it themselves; there is intentionally **no PR/cloud path** for docs review.
