...
- **`ingest/`** — Bento-based ingest pipeline (`bento.go`: JetStream input → per-table batch INSERT with DLQ output). The pipeline is **insert-only**. The wire format `EventMessage` (`types.go`) carries `{table_name, received_timestamp, data}` and nothing else; the worker validates the table name and the payload's presence, then bulk-INSERTs. In the embedded-NATS deployment (the default), the server runs with `DontListen: true` (`internal/mq/embedded.go`), so the only Publishers reachable on the `ingest.>` subjects are in-process Go code — today, only the HTTP `/v1/ingest/{table}` handler. A future `RemoteNATS` deployment would lose that perimeter (external publishers could reach `ingest.>`), which is the second motivation for the consumer-side defense-in-depth check described below. Non-insert mutations (`DELETE`/`UPDATE`/`TRUNCATE`/…) must go through `POST /v1/admin/query` under the admin/service role (the same gate as the rest of `/v1/admin/*`); when authentication is enabled, the `/v1/admin/*` middleware enforces that role check at the API layer, so non-admin callers never reach the proxy. With `auth.enabled=false` (or `auth.dev_mode=true`) the endpoint is intentionally open — the dev/test posture. As defense in depth that applies regardless of auth posture, the ingest worker also DoubleAck-drops any envelope on an `ingest.>` subject that carries an explicit `action` field set to anything other than `"insert"` — `EventMessage` has no `action` field today, so the legitimate producer never sets one, but a stale pre-deploy message could. Plus `Sweeper` (Active Sweeper for NATS message lifecycle) + `EventMessage`/`BufferConsumerName` types (`types.go`)
