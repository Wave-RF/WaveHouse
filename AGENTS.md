...
- **`policy/`** — Hasura-style access control: `Policy`/`TablePolicy` (role-first: `tables.<table>.<role>`)/`RolePermissions` with the per-operation `SelectPermissions`/`InsertPermissions` types, `Evaluate()` engine with JWT claim templating, `Source` (a `func() *Policy` read per call; `settings.Store.Policy` in production, `Static(p)` in tests)
...
3. **Schema-driven ingest** — `POST /v1/ingest?table={table}` takes flat JSON, validated against the discovered schema (unknown fields rejected, types/nullability enforced, `MATERIALIZED`/`ALIAS` columns refused — the server computes them). No envelope. `Content-Type` is **required and authoritative** (`application/json` or an NDJSON type); an undeclared or unreadable one is a `415` decided before the body is read, and a declared format is never overridden by the bytes.
