...
7. **Optional auth with JWKS**: JWT authentication is opt-in via `auth.enabled`. Supports HMAC shared secret and/or JWKS endpoint (`auth.jwks_url`). Roles are extracted from a configurable claim path (`auth.role_claim`). Dev mode (`auth.dev_mode`) bypasses validation for development. **Public (no-token) access is policy-driven, not a flag:** when auth is enabled, a request with **no** token is `401`'d unless the policy has a usable (non-`admin`/`service`) `default_role`, in which case it's admitted with an empty role that resolves to that `default_role`. The middleware decides admit-vs-`401` live via an injected `func() bool` wired to `policy.ResolveRole(store.Get(), "") != ""`, so a policy PUT/delete flips public access without a restart. A present-but-**invalid** token still `401`s; `/v1/admin/*` stays `admin`/`service`-only (`RequireRole`), and the token-only schema/DLQ routes stay gated by `RequireAuthenticated`; a pipe with no `allowed_roles` denies all callers except `admin`/`service`; and the policy cache is held fail-closed when auth is enabled (`Store.SetFailClosed` — a runtime KV delete is retained, not nilled) so `Evaluate`'s nil-policy allow-all can never expose tables to tokenless requests.
...
11. **Hasura-style access control**: Per-table, per-role column-level and row-level permissions with JWT claim templating (`{{ jwt.path }}`). Policies stored in NATS KV with file-based bootstrap and cluster-wide sync via KV Watch. An empty/absent role is fail-closed: it never matches a role key — matching is exact (there is no `"*"` any-role wildcard), and only the built-in `admin`/`service` roles bypass the allowlist — and `Validate` rejects empty role keys at write time — preserve this when touching `policy.Evaluate`/`Validate` (the policy twin of the pipe invariant in #13, see #159). The optional `default_role` is the one sanctioned exception: `policy.ResolveRole` maps an empty role to it *before* evaluation (so a roleless request gets that role's perms), but never to `admin`/`service` — that is refused in both `ResolveRole` (returns `""`, fail-closed) and `Validate` (rejects the write).
...
13. **Named query pipes**: Pre-defined SQL templates (inspired by Tinybird) with parameter binding, role restrictions, and caching. Stored in NATS KV with `.sql` file directory bootstrap. Per-pipe `allowed_roles` is the *only* authorization gate on the execute path — `GET/POST /v1/pipes/{name}` sit outside the `/v1/admin/*` `RequireRole` block — and it **fails closed**: authorization is exact allowlist membership (there is no `"*"` any-role wildcard), the built-in `admin`/`service` roles always pass, and any request whose resolved role is empty or absent (auth disabled, or a JWT missing `auth.role_claim`) matches nothing; empty-string allowlist entries are ignored so a stray `""` cannot authorize an empty role, and a pipe with no `allowed_roles` authorizes nobody but `admin`/`service`. When changing pipe execution, preserve this — and exercise it via the shared `testutil.RunRoleMatrix` / `StandardRoleMatrix` matrix (see #159).
