...
3. **Schema-driven ingest**: `POST /v1/ingest?table={table}` accepts a flat JSON body. The table name comes from the `table` query parameter. The body is validated against the discovered schema (unknown fields rejected, types checked, nullable constraints enforced). No envelope — just data. The same endpoint also accepts an **NDJSON batch** (`Content-Type: application/x-ndjson`, one JSON record per line): it fans into one independently-validated/published message per line and returns a per-record `{total,succeeded,failed,duplicates,errors}` summary, so one malformed/rejected line never fails the rest of the batch (whole-request conditions — table-level `insert` denial, backpressure, publish/dedup-backend errors — still abort non-200). Both paths share the `processRecord` helper in `internal/api/ingest.go`. Bounded-concurrency uploading of large batches is deferred to #196.
