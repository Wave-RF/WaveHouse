...
| Change to `EventMessage` / ingest event format | Update payload types in both SDKs (re-run the TS codegen CLI where types are generated; Go row structs are hand-written). |
...
| ClickHouse schema-driven type changes | Re-run the TS SDK codegen CLI and commit regenerated types; Go row structs are hand-written by the consumer. |
...
clients/go/             → Go SDK (github.com/Wave-RF/WaveHouse/clients/go) — nested module; file layout mirrors clients/ts/, plus testdata/wire_cases.json
