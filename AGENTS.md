...
17. **Non-fatal boot**: Schema-discovery failure on boot (ClickHouse unreachable, missing database, transient network blip) is non-fatal — `cmd/wavehouse` records an `api.BootState` diagnostic, binds `:8080`, and retries via `SchemaRegistry.RetryRefresh` (exp backoff 2s → 60s) in the background. `/healthz` and `/readyz` return 503 with the latest diagnostic until a Refresh succeeds, after which `/healthz` stays 200 for the rest of the process. Keeps supervisor restart loops bounded and gives operators a queryable failure surface (`curl /healthz`) instead of a restart-log grep.
18. **Health endpoint convention**: Liveness is `/healthz` and readiness `/readyz` (Kubernetes convention); `/health` and `/ready` are **deprecated aliases** of the same handlers, kept for v0.1.x and slated for removal in v0.2.0 (see CHANGELOG #144). Response bodies are unchanged from the old paths — `/healthz` returns `{"status":"ok"}` (or `{"status":"degraded",…}` + 503 while boot is still failing) and `/readyz` returns `{"status":"ready"}` (or `{"status":"not ready","error":…}` + 503). Point new tooling at `/healthz`/`/readyz`, never the aliases. (Per-dependency drill-down probes and a richer aggregate `/readyz` body — issue #144 part 2 — were deferred to keep this change small.)
