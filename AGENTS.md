...
13. **Named query pipes**: Pre-defined SQL templates (inspired by Tinybird) with parameter binding, role restrictions, and caching. Stored in NATS KV with `.sql` file directory bootstrap. Per-pipe `allowed_roles` is the *only* authorization gate on the execute path — `GET/POST /v1/pipes/{name}` sit outside the `/v1/admin/*` `RequireRole` block — and it **fails closed**: a non-empty `allowed_roles` denies any request whose resolved role is empty or absent (auth disabled, or a JWT missing `auth.role_claim`) unless the list contains `"*"`, and empty-string allowlist entries are ignored so a stray `""` cannot authorize an empty role. When changing pipe execution, preserve this — and exercise it via the shared `testutil.RunRoleMatrix` / `StandardRoleMatrix` matrix (see #159).
