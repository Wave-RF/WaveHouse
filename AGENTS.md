...
- **`keyenc/`** — the one escaping composite keys are built from: `Escape` keeps `[A-Za-z0-9_]` and writes every other byte as `%XX`, `Unescape` reverses it leniently (as `url.PathUnescape` does), `Join`/`Split` join escaped fields with a separator the escaping never emits. NATS subject tokens and the cache's namespace tokens use it; its output is pinned byte for byte, since v0.1.0's subjects carry it
