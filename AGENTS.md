...
- **`observability/`** — OpenTelemetry pipeline: `InitProvider` wires trace/metric/log providers via OTLP gRPC (each signal independently gated). A top-level `Prometheus` config block drives an optional `/metrics` scrape endpoint that runs independently of OTLP push — standalone (Alloy/Mimir scrape, no collector), alongside OTLP, or off. `NewLogger` produces a slog handler that fans out to stdout AND OTLP (stdout always 100%, OTLP sample-rate-aware) at the configured `WH_LOG_FORMAT` (auto-TTY / text / json). `TraceHandler` injects `trace_id`/`span_id` from active spans AND a per-request `component` from `observability.WithComponent(ctx, "<area>")` (e.g. `api/ingest`, `ingest/sweeper`). `tracer.go` provides W3C trace context propagation over NATS headers. `instruments.go` is the single registry for cross-cutting metric instruments (latency histograms, error/saturation counters, per-tier cache counters).
