...
12. **Structured queries**: Type-safe query AST endpoint (`POST /v1/query?table={table}`) validated against schema, with permission enforcement, timestamp bucketing for cache optimization, and `DefaultMaxRows` (10,000) limit cap. **Column authorization is enforced inside the SQL builder (`query.Build`), the single chokepoint that already enumerates every column reference** — projection, aggregation arguments, `filters`, `group_by`, `order_by`, `time_range` — so no clause can skip the role's `allow_columns`/`deny_columns` check. A full-row read (`select_all: true`) by a *column-restricted* role expands to that role's allowed columns via `policy.AllowedProjection` instead of emitting a bare `SELECT *`; *unrestricted* roles and the admin role keep `SELECT *` (`policy.RestrictsColumns` decides which). Omitting `columns` (or sending `[]`/`""`) selects nothing (`query.ErrEmptyProjection` → `200 []`) and `["*"]` is the literal column named `*` (not a wildcard), so neither implicitly expands to `SELECT *`. A role granted the table but no readable columns fails closed (`query.ErrNoReadableColumns` → `403`), never `SELECT *`. The structured-query path and the live-stream path (`filterEventColumns`) both defer to the one per-column decision `policy.IsColumnAllowed` (`IsColumnAllowed("*")` is always false — a literal `"*"` is resolved by the projection layer, not waved through), so the two read surfaces enforce identical column visibility and can't drift. **Preserve this when touching `internal/query` or the structured-query handler** — scattering the column checks back into the handler is exactly what produced the #223 fail-open: it checked only the SELECT list and aggregations, leaving omitted/`"*"` columns (→ `SELECT *`), `group_by`, `order_by`, and `filters` able to read or infer denied columns.
