...
- **`ingest/`** — Bento-based ingest pipeline (`bento.go`: JetStream input → per-table batch INSERT with DLQ output). The pipeline is **insert-only**. Accepted envelopes: `action: "insert"`, *or* `action` absent/empty. Absent is accepted because the wire format `EventMessage` (`types.go`) has no `action` field — `internal/api/ingest.go` builds the envelope as `{table_name, received_timestamp, data}` with no action set, so every envelope from the HTTP API path arrives at the worker with the field absent. Any other `action` value (`delete`, `update`, `truncate`, `drop`, `alter`, `replace`, anything else — produced today only by external NATS publishers) is `DoubleAck`'d and dropped; the corresponding mutations must go through `POST /v1/query` under an admin-equivalent role. Plus `Sweeper` (Active Sweeper for NATS message lifecycle) + `EventMessage`/`BufferConsumerName` types (`types.go`)
