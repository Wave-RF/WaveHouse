...
| `filter` | map | select | Row-level `WHERE` predicates (`_eq`/`_neq`/`_gt`/`_lt`/`_in`), ANDed together. `_in` takes a single array-valued claim → `col IN (…)`. Values support `{{ jwt.path }}` templating; a claim the token doesn't carry fails the filter closed (no rows), and a malformed template is rejected when the policy is written. |
