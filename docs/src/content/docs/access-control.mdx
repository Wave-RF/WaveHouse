...
One admin endpoint (requires `admin_role`) reads it:
...
```bash
# Check the edited settings directory before the server sees it:
wavehouse validate ./settings

# Then let the watcher adopt the change, or force it:
curl -X POST http://localhost:8080/v1/ops/settings/reload \
  -H "Authorization: Bearer $ADMIN_TOKEN"
```

The full request and response shapes for these endpoints live in the [API Reference](/api); the [TypeScript SDK](/sdk) wraps them as `client.policy.get()` and `client.settings.reload()`.
