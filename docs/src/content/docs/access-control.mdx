...
> The policy `Filter` schema also accepts `_in`, but it is **not yet enforced** (tracked in [#224](https://github.com/Wave-RF/WaveHouse/issues/224)) — an `_in` clause is silently ignored (no predicate is produced), so stick to the operators above.
...
Four fields cap the cost of a single structured query. All must be non-negative; `0` (the default) means "no role-imposed limit" — the server-wide [`query_limits` defaults](/configuration#query-limits) still apply (see below).

| Field | Effect |
| ----- | ------ |
| `max_rows` | Caps the query's `LIMIT`. If the caller asks for more (or omits a limit), the result is clamped to `max_rows`. |
| `max_execution_time` | A duration string (`"5s"`, `"500ms"`) capping the query execution time, applied as the *minimum* of this value and the server's `clickhouse.query_timeout`. |
...
| `max_memory_usage` | A size string (`"4GiB"`, `"512MiB"`) capping **peak query memory** server-side — the lever that stops a heavy aggregation from exhausting the box. The read is rejected once it is exceeded. |

`max_rows_to_read`, `max_memory_usage`, and the execution-time budget are enforced **server-side by ClickHouse** (sent as per-query `max_rows_to_read` / `max_memory_usage` / `max_execution_time` settings), so a query can't outrun its budget during a server-side scan, merge, or aggregation phase — not just while the client is reading rows back. `max_rows` is applied as the SQL `LIMIT` (and mirrored server-side as `max_result_rows` for defense-in-depth).
...
A role that sets no cap of its own still gets a safety net: the [`query_limits` config block](/configuration#query-limits) (`default_max_rows`, `default_max_rows_to_read`, `default_max_memory_usage`) applies its rows-scanned and memory budget to **every non-admin read — structured queries *and* named pipes**, since pipes carry no per-role caps. A per-role policy cap overrides the default for that table+role; the **admin role bypasses these resource backstops entirely**. The 4 GiB default memory cap is on out of the box; the rows-scanned default is off (`0`) by default.
...
| ------- | -------- | -------- |
| Structured read | `POST /v1/query?table={table}` | table+role `select` required, then `allow`/`deny_columns`, row `filter`, aggregation rules, `max_rows`, `max_execution_time`, `max_rows_to_read`, `max_memory_usage` (the last three over the [`query_limits` defaults](/configuration#query-limits)) |
...
| Named pipe | `GET/POST /v1/pipes/{name}` | per-pipe `allowed_roles` (not the policy engine; see [Named Pipes](/pipes)), plus the server-wide `query_limits` rows-scanned/memory defaults for non-admin reads |
...
| `max_rows` | int | select | Caps the query `LIMIT`. `0` = no role limit (the `query_limits.default_max_rows` default applies). Must be non-negative. |
| `max_execution_time` | duration string | select | Caps the query timeout (min with `clickhouse.query_timeout`), e.g. `"5s"`, `"500ms"`. Empty = no limit. Must be non-negative. |
| `max_rows_to_read` | int | select | Caps rows **scanned** server-side (ClickHouse `max_rows_to_read`); the read is rejected once exceeded. `0` = inherit `query_limits.default_max_rows_to_read`. Must be non-negative. |
| `max_memory_usage` | size string | select | Caps peak query memory server-side (ClickHouse `max_memory_usage`), e.g. `"4GiB"`, `"512MiB"`; the read is rejected once exceeded. Empty/`0` = inherit `query_limits.default_max_memory_usage`. Must be non-negative. |
