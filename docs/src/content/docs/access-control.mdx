...
Four fields cap the cost of a single structured query. All must be non-negative; `0` (the default) means "no policy-imposed limit".

| Field | Effect |
| ----- | ------ |
| `max_rows` | Caps the query's `LIMIT`. If the caller asks for more (or omits a limit), the result is clamped to `max_rows`. |
| `max_execution_time_ms` | Caps the query execution time, applied as the *minimum* of this value and the server's `clickhouse.query_timeout`. |
...
select:
  viewer:
    ...
    max_memory_usage_bytes: 4294967296  # 4 GiB peak memory per query
```
  </TabItem>
  <TabItem label="JSON">
```json
{
  "select": {
    "viewer": {
      ...
      "max_memory_usage_bytes": 4294967296
    }
  }
}
```
...
| ------- | -------- | -------- |
| Structured read | `POST /v1/query?table={table}` | table+role `select` required, then `allow`/`deny_columns`, row `filter`, aggregation rules, `max_rows`, `max_execution_time_ms`, `max_rows_to_read`, `max_memory_usage_bytes` |
...
SSE subscribers are checked for table-level `select` permission and have denied columns stripped from each event, but the row-level `filter` predicates and the resource limits (`max_rows`, `max_execution_time_ms`, `max_rows_to_read`, `max_memory_usage_bytes`) are a property of the SQL query path and are **not** applied to the live event stream. If a role must never observe another tenant's rows in real time, don't grant it stream access to a shared table — scope the data at the table level.
:::
...
| `max_rows_to_read` | int | select | Caps rows **scanned** server-side (ClickHouse `max_rows_to_read`); the read is rejected once exceeded. `0` = no limit. Must be non-negative. |
| `max_memory_usage_bytes` | int | select | Caps peak query memory server-side (ClickHouse `max_memory_usage`, in bytes); the read is rejected once exceeded. `0` = no limit. Must be non-negative. |
