...
**A `check` on a column the table cannot accept fails at run time, not at validation.** Policy validation never sees the ClickHouse schema, so it cannot tell that a checked column is missing from the table or is one the server computes (`MATERIALIZED`/`ALIAS`, which no record may write). Either way the required value could never reach the stored row, so ingest refuses the record with `403 policy check references column "x", …` — on **every** insert by that role, until the policy or the table is corrected. Check the column names in a `check` against the table when you write one; `wavehouse validate` will not.
