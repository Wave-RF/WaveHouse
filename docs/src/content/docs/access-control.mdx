...
Unlike row filters, an unresolvable claim in an `_eq` check is **not** fail-closed: it still resolves to the empty string, so an omitted column is auto-injected as `''`, and any supplied value other than `''` is rejected with `403 check failed`. (An unresolvable `_in` check *does* fail closed — the claim-derived set is empty, so every insert to that column is rejected.) Check-path semantics are being standardized in [#371](https://github.com/Wave-RF/WaveHouse/issues/371).
