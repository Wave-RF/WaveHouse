...
- It is the gate for every admin surface: `/v1/admin/*` (raw SQL, policy CRUD, pipe CRUD, runtime settings), plus the schema and DLQ endpoints. The gate reads `admin_role` live from the policy, so changing it applies without a restart.
...
The control-plane database is the **source of truth** for the live policy. A file is only ever a *seed* for an empty store:

- `policy.file_path` (config) points at a YAML or JSON policy file. On startup, **if and only if the database holds no policy**, the file is loaded, validated, and written to the database. On every subsequent boot the file is ignored — the database already has the authoritative copy, and runtime edits flow through `PUT /v1/admin/policy`.
...
- **When `policy.file_path` is empty and the database holds no policy**, the store comes up with no policy — every request is denied (logged loudly) until you seed one via `PUT /v1/admin/policy`.
...
```yaml
policy:
  # seed on first boot only; the control-plane db wins thereafter
...
Because a deleted policy locks out every *token-based* caller (including admin), treat the bootstrap file as your recovery path — unless you configured an [operator key](#operator-key), which restores a policy over HTTP without a reboot. Keep the bootstrap file in version control, mount it read-only, and you can always restore a known-good policy by stopping WaveHouse, removing `<data_dir>/control.db` **and its SQLite `-wal`/`-shm` sidecars** when present (back the three files up first if you might need the old state), then restarting so bootstrap runs against a fresh database — this also resets pipes and runtime settings, since the database holds all three. Delete only with the server stopped: the running process keeps the old database open, and a leftover sidecar can carry stale control-plane writes into the next boot. See [Configuration — Access Control Policy](/configuration#access-control-policy) for the exact knobs.
