...
- **discovery.go** — `SchemaRegistry` queries `system.columns` to discover ClickHouse table schemas, keeping each column's `default_kind`, `default_expression` and `position` alongside its name and type. `default_kind` is what decides insertability: a `MATERIALIZED` or `ALIAS` column cannot be named in an INSERT, so `InsertableColumns` is the subset the ingest wire shape is built over, while `Columns` stays whole for the schema endpoint and the query path. Each refresh also scans `system.tables` for every table's `create_table_query` (kept in memory and deliberately never serialized — an external-engine table carries its credentials there), records the server version (`SELECT version()`), and discovers the server's default time zone (`SELECT timezone()`) and bakes every `DateTime`/`DateTime64` column's canonicalization spec (precision + resolved zone) into the cached schema, so the per-record ingest path parses no type strings and loads no zones ([#372](https://github.com/Wave-RF/WaveHouse/issues/372)). Supports periodic auto-refresh, on-demand refresh, and `RetryRefresh` (boot-time exponential backoff loop used by `cmd/wavehouse` so a transiently unreachable ClickHouse doesn't crash-loop the binary). Thread-safe via `sync.RWMutex`.
...
Client POST /v1/ingest?table={table}
  ...
  → Resolve the declared Content-Type to a format — 415 when absent or
    unsupported, decided before a byte of the body is read. The bytes pick
    array-vs-object WITHIN the JSON family; they never choose the format
  → Read the (capped) body in full, then validate each record against the
    schema (type checks, required columns, computed columns refused)
...
Ingest worker pipeline (StartIngestWorker):
  ...
  → Parse the event envelope (poison: malformed JSON, an unknown format, or
    columns/row that don't pair — parked on the DLQ, or acked-and-dropped and
    counted where the DLQ is off for the table)
  ...
  {table_name, scope, received_timestamp, format, columns, row};
  non-insert mutations
...
Client GET /v1/stream
  → JWT auth middleware (always runs; token optional)
  → Announce the projected column list: `event: schema` from the registry,
    before any row (rows are positional, so a row is unreadable without it)
