...
Client GET /v1/stream
  ...
  → Announce the caller's projected column list as an `event: schema` frame
    (no `id:`, so it never moves Last-Event-ID) BEFORE registering, so the
    LIVE path can never hand a client a row without the names for its slots
