...
- **dlq.go** — DLQ stats endpoint (`GET /v1/ops/dlq/stats`): asks `mq.DeadLetterStats.DeadLetterCounts` for the per-table parked counts (optionally one table) and the total. A dead-letter queue that cannot be resolved (`mq.ErrNoDeadLetterQueue`) reads as empty; a failed read of it is a 500. The queue itself is `internal/mq`'s.
