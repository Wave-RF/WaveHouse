...
- **stream.go** — Real-time streaming via SSE. Callers select a table with the `?table=` query parameter. Supports gap-fill from NATS JetStream using `DeliverByStartTime`. Each connection registers with the shared `Heartbeater` (see `heartbeat.go`) so idle streams keep emitting `: heartbeat` comments and survive reverse-proxy idle timeouts.
- **heartbeat.go** — Shared SSE keepalive: one process-wide `Heartbeater` ticker fans `: heartbeat` comments out across a ring of `Bucket`s, waking ~1/N of live streams per tick so the writes don't synchronize. Per-connection period is `heartbeat_interval × heartbeat_buckets`; the owning handler goroutine does the actual write, so the shared ticker never touches a `ResponseWriter` directly.
