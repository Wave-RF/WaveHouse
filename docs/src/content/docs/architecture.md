...
- **metrics.go** — `Metrics`, the SSE instrument set: `wavehouse_sse_active_streams` (open streams), `wavehouse_sse_stream_duration_seconds` (lifetime), and `wavehouse_sse_frames_sent_total` / `wavehouse_sse_bytes_sent_total` (labeled `kind=keepalive` vs `kind=event`). Nil-safe, so the handler holds one unconditionally and tests skip wiring it; recorded at the handler's connect/disconnect and write sites. Separate from `observability.RegisterSystemMetrics`, which covers only the NATS/Pebble system gauges. Streams are observed through these metrics rather than per-event traces (the router excludes `/v1/stream` from the HTTP tracer).
