...
- **nats.go** — `SafeEncodeNATS`/`SafeDecodeNATS`, the single table-name → NATS-safe-token encoder shared by every side of cache invalidation: the ingest write side (subjects and `Invalidate` namespaces), the pipe/structured-query read side (dependency namespaces folded into cache keys), and the registry's base→view cascade. Alphanumerics and `_` pass through; everything else percent-encodes, so any ClickHouse-legal name round-trips and both sides always build identical namespace keys.
