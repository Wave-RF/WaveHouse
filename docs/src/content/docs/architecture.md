...
Bento ingest pipeline (StartIngestWorker):
  ...
  the endpoint is intentionally open). As defense in depth that applies
  regardless of auth posture, the worker also DoubleAck-drops any envelope
  that carries an explicit non-`insert` `action`.)
