...
- **metrics.go** — `RegisterSystemMetrics(mqStats, dedup)` registers observable gauges for embedded NATS connections, in-msgs, and Pebble dedupe storage stats. `mqStats` is a `func() (MQStats, error)` read on every scrape (`mq.Broker.Stats` in production; nil skips the MQ gauges) — this package never holds the NATS server. Wired in `internal/app` after the providers are up.
