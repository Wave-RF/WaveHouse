...
- **metrics.go** — `RegisterSystemMetrics(natsServer, dedup, chConn, database)` registers observable gauges for embedded NATS connections, in-msgs, Pebble dedupe storage stats, and per-table ClickHouse storage: `wavehouse_clickhouse_uncompressed_bytes` / `wavehouse_clickhouse_bytes_on_disk` (labeled by `table`) scrape one aggregate over `system.parts` (active parts, configured database) — live data before vs. after compression, so instance totals and the compression ratio are dashboard-layer sums/quotients over the `table` attribute. Wired in `cmd/wavehouse/main.go` after the providers are up.
