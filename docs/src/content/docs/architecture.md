...
Ingest worker pipeline (StartIngestWorker):
  ...
  (Insert-only pipeline. The wire format `EventMessage` carries
  {table_name, scope, received_timestamp, data}, scope today always
  empty; non-insert mutations
...
Client POST /v1/ops/query
  ...
  → /v1/ops RequireAdmin (role == policy.admin_role, or the operator-key bit) — single gate shared
    with the rest of /v1/ops/* (policy CRUD, pipes CRUD). Raw SQL has
