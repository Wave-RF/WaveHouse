...
- **check.go** — `UnboundEnv` walks the struct's `env` tags (plus the two process-level names, `WH_CONFIG` and `WH_LOG_LEVEL`) against the environment; `CheckDataDir` probes that `data_dir` is a writable directory, or creatable under a writable parent, by creating and removing one temp file — run by `main` right after `Load`, so a root-owned bind mount refuses boot before anything dials out, with the UID-65532 hint attached.
