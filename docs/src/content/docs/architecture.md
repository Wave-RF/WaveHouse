...
- **dlq.go** — DLQ stats endpoint (`GET /v1/ops/dlq/stats`): resolves the `WAVEHOUSE_DLQ` stream through `mq.StreamManager` and reads its per-subject counts off `mq.StreamState`. The stream itself is created by `mq.NewEmbedded` and resized on reload by `mq.EmbeddedNATS.SetMaxBytes`.
