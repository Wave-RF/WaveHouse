...
- **metrics.go** — `RegisterSystemMetrics(natsServer, dedup, chConn, database)` registers observable gauges for embedded NATS connections, in-msgs, Pebble dedupe storage stats, and per-table ClickHouse storage: `wavehouse_clickhouse_uncompressed_bytes` / `wavehouse_clickhouse_bytes_on_disk` (labeled by `table`) scrape one aggregate over `system.parts` (active parts, configured database) — uncompressed data vs. total on-disk footprint (compressed data plus marks/index files), so instance totals and the footprint-inclusive compression ratio are dashboard-layer sums/quotients over the `table` attribute. The scrape is bounded at 5s and observes all-or-nothing: on error or timeout the storage series go absent for that cycle, never stale or partial. Wired in `cmd/wavehouse/main.go` after the providers are up; operator-facing guidance (alerting, scope, failure semantics) in [Deployment → Observability](/deployment#per-table-storage-gauges).
