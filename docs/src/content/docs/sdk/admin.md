...
Over [a nested settings directory](/deployment#the-nested-settings-directory), pass `tenant` to read or refresh that tenant's schema; without it the calls address tenant `0`. A `503` with `Retry-After` is a tenant whose first discovery has not succeeded yet (the SDK retries it), and `wh.sql()` takes the same option to run against that tenant's ClickHouse:
