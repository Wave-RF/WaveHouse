...
Neither SDK has a first-class operator-key option; each carries it as a custom header — [`options.headers`](/sdk/typescript#custom-headers) in TypeScript, [`ClientOptions.Headers`](/sdk/go#clientoptions) in Go.
