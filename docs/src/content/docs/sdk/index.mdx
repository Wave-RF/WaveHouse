...
WaveHouse also ships an official Go SDK (`github.com/Wave-RF/WaveHouse/clients/go`) — zero third-party dependencies, `context.Context`-first, generics for typed rows. See the [Go SDK docs](/sdk/go). Both clients speak the same wire format, so everything below about tables, the query builder, streaming, and admin endpoints carries over conceptually; the [API shapes differ](/sdk/go#differences-from-the-typescript-sdk).
:::
...
<CardGrid>
  ...
  <LinkCard
    title="Go SDK"
    description="The zero-dependency Go client — context.Context, generics for typed rows, and a codegen CLI."
    href="/sdk/go"
  />
</CardGrid>
