...
The SDK attaches your `auth` token as an `Authorization: Bearer` header on REST calls, and — because the browser `EventSource` API can't set headers — as a `?token=` query parameter on streaming connections. When both are present the server prefers the header, and strips the `?token=` value after extraction so it stays out of **WaveHouse's own** logs.

That strip can't reach anything upstream, though: on a streaming connection the token is in the request URI on the wire, so any proxy, CDN, or load balancer in front of WaveHouse will record it in access logs unless you redact query strings there.
:::

#### Serving under a path prefix

`baseURL` may carry a path prefix, for a WaveHouse reachable somewhere other than the root of an origin — behind a BFF, an app-server route, or a path-routed ingress. Every request path is appended to it, on both transports:
