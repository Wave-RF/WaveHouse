...
| `options.maxRetries` | `number` | `2` | Retry attempts for retryable **REST** failures — network errors and 5xx, unless the server's body says `retryable: false`; stream reconnects are unbounded ([details](/sdk/streaming#transport-behavior)) |
