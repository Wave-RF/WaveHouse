...
The SDK attaches your `auth` token as an `Authorization: Bearer` header on REST calls, and — because the browser `EventSource` API can't set headers — as a `?token=` query parameter on streaming connections. When both are present the server prefers the header. Either way it strips `?token=` from the URL after extraction — whichever credential won — so no token stays in `r.URL` for a later handler to log, and it stays out of **WaveHouse's own** logs.
