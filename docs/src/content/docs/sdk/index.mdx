...
The SDK attaches your `auth` token as an `Authorization: Bearer` header on REST calls, and — because the browser `EventSource` API can't set headers — as a `?token=` query parameter on streaming connections. When both are present the server prefers the header and leaves the query parameter untouched; when the token arrives *only* as `?token=`, the server strips it from the URL after extraction so it stays out of **WaveHouse's own** logs.

Either way, that strip can't reach anything upstream: the token is in the request URI on the wire, so any proxy, CDN, or load balancer in front of WaveHouse records it in access logs unless you redact query strings there.
:::
...
Trailing slashes on `baseURL` are optional, but it must be **absolute** — scheme and host included. A same-origin relative path like `/api/warehouse` throws `TypeError: Invalid URL` at the first request instead of returning a `Result`, so build an absolute one: `` `${location.origin}/api/warehouse` ``.
...
Every async SDK operation returns `Result<T>` — a discriminated union that never throws for anything the server returns (a malformed `baseURL` is the one exception, and fails fast):
