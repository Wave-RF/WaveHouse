...
a keep-alive socket avoids the stall — `new Agent({ pipelining: 0 })`, passed as
`dispatcher` in the `RequestInit`. It costs you a fresh connection per request,
so prefer upgrading. Note that tuning `keepAliveTimeout` does **not** help: the
retirement timer is starved by the same idle event loop that triggers the bug,
so the socket is still there to be reused.
