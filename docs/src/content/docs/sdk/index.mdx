...
| `options.fetch` | `typeof fetch` | global `fetch` | HTTP implementation used for every request |
...
// e.g. retire idle sockets sooner than the runtime's default
...
Retries go through the same function, so middleware sees every attempt. Only the
request path is affected — streaming (`.stream()`, `.liveQuery()`) uses
`EventSource`, which this does not replace.
