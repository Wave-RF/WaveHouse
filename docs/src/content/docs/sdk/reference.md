...
| 503 | `HTTP_503` | Yes | Service unavailable, a tenant whose settings folder was rejected, or a token sent while that tenant's JWKS has not been fetched yet (`token verifier not ready`, `Retry-After: 30` — so each attempt waits the 30 s before retrying; auto-retries, honoring `Retry-After` when the response carries one) |
