...
- **Parameters never decide the format.** `application/json; charset`, `application/json;;`, `application/json ; charset=utf-8` all read as `application/json` — a malformed parameter does not cost you the request. Two exceptions, both because they leave no usable media type: a duplicate parameter name (`; charset=a; charset=b`), and any of the above on a line that *also* carries a comma.
- **A comma inside a quoted parameter value is data**, not a separator (RFC 9110 §5.6.6), so `application/json; profile="a,b"` is one media type and is accepted.
- **Declaring the type more than once is malformed.** `Content-Type` is a *singleton* field (RFC 9110 §8.3, `Content-Type = media-type`), and §5.3 forbids repeating a field line unless the field allows comma-list recombination — `media-type` does not. §8.3 warns that resolving the resulting pseudo-list by taking "the last syntactically valid member" causes "interoperability and security issues", so ingest takes no member: a **comma-joined** value is rejected, and **repeated header lines** are all resolved and must **agree**. One limit, inherent to joining being lossy: if the joined line's quotes happen to balance, it *is* one valid media type by the grammar (`application/json; a=", application/x-ndjson; b="`) and is read as one, where the two-line spelling would be refused. The reverse also occurs — a joined line whose quotes do not balance is refused where the two-line spelling is accepted ([#563](https://github.com/Wave-RF/WaveHouse/issues/563)).
- **Agreement is on the resolved format, not the text.** `application/x-ndjson` and `application/ndjson; charset=utf-8` agree, and the request is read as NDJSON. They disagree when they name different formats, or when one names a type ingest reads and the other does not (`application/json` and `text/csv`) — rejected rather than resolved to the first, since honoring the first would let an NDJSON body be read as a single JSON object and drop every record after it. An empty line (a bare `Content-Type:`) names no type, so it disagrees with any real declaration.
