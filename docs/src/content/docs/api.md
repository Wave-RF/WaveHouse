...
**`Content-Type` is required and authoritative.** The format is what the caller declares, not what the bytes look like: a body declared as NDJSON is read as NDJSON whatever its first byte, so a line that isn't a JSON object fails as a per-record error rather than silently re-framing the whole request. A request with **no** `Content-Type`, or one whose media type is not in the accepted list, is rejected with `415` and a message listing the accepted types — nothing is guessed. Parameters are ignored outright, so no parameter shape short of one containing a comma can cost you the request: `application/json; charset`, `;;`, and a repeated `; charset=…` all read as `application/json`. One exception, and it is a refusal: a header containing a **comma** is two declarations joined into one (what a proxy does with duplicate `Content-Type` headers), so it is rejected rather than resolved to the first — `application/json; charset=utf-8, application/x-ndjson` would otherwise read an NDJSON body as a single JSON object and drop every record after the first. The check is a plain comma test anywhere in the header, so a comma inside a quoted parameter value is refused as well. Sending the header **twice** is refused on the same grounds when the two disagree; repeating an identical declaration is fine. The one thing the body still decides is *arity within the JSON family*: the first non-whitespace byte picks a top-level array (`[`) or a single object.
