...
**`Content-Type` is required and authoritative.** The format is what the caller declares, not what the bytes look like: a body declared as NDJSON is read as NDJSON whatever its first byte, so a line that isn't a JSON object fails as a per-record error rather than silently re-framing the whole request. A request with **no** `Content-Type`, one whose media type cannot be read at all (`application/json, application/json`, which is what a proxy leaves behind when it joins two `Content-Type` headers), or one whose media type is not in the accepted list, is rejected with `415` and a message listing the accepted types — nothing is guessed. A malformed *parameter* is not grounds for rejection: `application/json; charset` reads as `application/json`. The one thing the body still decides is *arity within the JSON family*: the first non-whitespace byte picks a top-level array (`[`) or a single object.
