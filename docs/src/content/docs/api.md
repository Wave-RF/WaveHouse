...
- An **integer** JSON number — read the way ClickHouse reads numbers: Unix seconds for `DateTime`, **ticks at the column's scale** for `DateTime64` (`1750478400500` into a `DateTime64(3)` is the millisecond epoch). Non-integer numbers pass through; ClickHouse rejects them.
