...
**`Content-Type` is required and authoritative.** The format is what the caller declares, not what the bytes look like: a body declared as NDJSON is read as NDJSON whatever its first byte, so a line that isn't a JSON object fails as a per-record error rather than silently re-framing the whole request. A request with **no** `Content-Type`, or one whose media type is not in the accepted list, is rejected with `415` and a message listing the accepted types — nothing is guessed. Parameters are ignored outright, so no parameter shape short of one containing a comma can cost you the request: `application/json; charset`, `application/json;;`, `application/json ; charset=utf-8`, and a repeated `; charset=…` all read as `application/json`. You may end up declaring the type more than once — an intermediary can repeat the `Content-Type` header line, or merge duplicates into one line separated by a **comma**. Both spellings mean the same thing and get the same rule: every declaration is resolved, and they must **agree**. Agreement is on the resolved format, not the text, so `application/x-ndjson` and `application/ndjson; charset=utf-8` agree and the request is read as NDJSON. Declarations disagree when they name different formats (`application/json` and `application/x-ndjson`) or when one names a type ingest reads and the other does not (`application/json` and `text/csv`); either way the request is rejected rather than resolved to the first, because honoring the first would let an NDJSON body be read as a single JSON object and drop every record after it. A comma inside a quoted parameter value is split like any other, so such a header is rejected whenever the parts disagree — `application/json; foo="x,y"` is. An **empty** declaration — a bare `Content-Type:` line, or the trailing element of `application/json,` — is ignored rather than counted as disagreement, since it contradicts nothing; a request with nothing but empty declarations is the same as one with no header at all. The one thing the body still decides is *arity within the JSON family*: the first non-whitespace byte picks a top-level array (`[`) or a single object.
...
A batch aborted partway (a `503`/`500`, or a JSON-array syntax error, after some leading records were already published) re-publishes those leading records when the whole batch is retried. A `413` is **not** one of these: the body is read in full before any record is processed, so an over-cap request publishes nothing and is safe to retry once split. Enable deduplication if duplicate suppression matters — this is the same at-least-once property the single-object path already has (the SDK retries both on `503`).
:::
