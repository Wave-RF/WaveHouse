...
:::note[Duplicate or malformed declarations]

- **Parameters never decide the format.** `application/json; charset`, `application/json;;`, `application/json ; charset=utf-8`, and a repeated `; charset=…` all read as `application/json`. No parameter shape short of one containing an *unquoted* comma can cost you the request — with one exception: once a header line ends mid-quote, every comma on that line loses its quoting, including one inside a well-formed value elsewhere on the same line; see the comma bullet below.
- **You may declare the type more than once** — an intermediary can repeat the `Content-Type` header line, or merge duplicates into one line separated by a **comma**. Both spellings mean the same thing and get the same rule: every declaration is resolved, and they must **agree**. (One limit: this holds while every declaration's own quotes balance. Once any of them ends mid-quote the two spellings can answer differently, in either direction.)
- **Agreement is on the resolved format, not the text.** `application/x-ndjson` and `application/ndjson; charset=utf-8` agree, and the request is read as NDJSON.
- **Declarations disagree** when they name different formats (`application/json` and `application/x-ndjson`) or when one names a type ingest reads and the other does not (`application/json` and `text/csv`). Either way the request is rejected rather than resolved to the first — honoring the first would let an NDJSON body be read as a single JSON object and drop every record after it.
- **A comma inside a quoted parameter value is data**, not a separator (RFC 9110 §5.6.6), so `application/json; profile="a,b"` **on its own** is one declaration. The closing quote is load-bearing: an unterminated one has no valid parse, so nothing anywhere in *that header line* is protected — not just the offending parameter. `application/json; profile="a,b` is refused, and so is `application/json; profile="a,b", application/json; x="`, where the well-formed `profile="a,b"` is torn apart by its mid-quote neighbor; the same pair sent as two header lines is accepted. Narrowing that is tracked in [#563](https://github.com/Wave-RF/WaveHouse/issues/563).
- **An empty declaration is ignored** rather than counted as disagreement — a bare `Content-Type:` line, or the trailing element of `application/json,`. A request with nothing but empty declarations is the same as one with no header at all.
...
| 415 | `{"error":"no Content-Type: ingest requires one of application/json, application/x-ndjson, …"}` (declared variant: `Content-Type "text/plain": ingest requires one of …`, naming every declaration when several were sent; conflicting variant: `conflicting Content-Type declarations "application/json", "application/x-ndjson": ingest reads one format per request, and requires one of …`) | The request declared no `Content-Type`, an unsupported one, or declarations that disagree — different formats, or one supported and one not. Checked before the body is parsed |
