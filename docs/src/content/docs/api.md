...
- **Parameters never decide the format.** `application/json; charset`, `application/json;;`, `application/json ; charset=utf-8`, and a repeated `; charset=…` all read as `application/json`. No parameter shape short of one containing an *unquoted* comma can cost you the request.
- **You may declare the type more than once** — an intermediary can repeat the `Content-Type` header line, or merge duplicates into one line separated by a **comma**. Both spellings mean the same thing and get the same rule: every declaration is resolved, and they must **agree**. (One limit: two lines that *each* end mid-quote balance only once joined, and that boundary cannot be recovered — so those two spellings can answer differently.)
...
- **A comma inside a quoted parameter value is data**, not a separator (RFC 9110 §5.6.6), so `application/json; profile="a,b"` is one declaration. The closing quote is load-bearing: an unterminated one has no valid parse, so nothing in the value is protected and `application/json; profile="a,b` is refused.
