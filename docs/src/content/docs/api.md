...
The header is parsed exactly as RFC 9110 defines it (§8.3 `media-type`), so the grammar's rules apply unchanged — notably that parameters are optional and never affect the format, so a malformed one doesn't cost you the request, and that a comma inside a quoted parameter value is data rather than a separator (`application/json; profile="a,b"` is one media type).
