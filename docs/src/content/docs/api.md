...
| 415 | `{"error":"no Content-Type: ingest requires one of application/json, application/x-ndjson, …"}` (declared variant: `Content-Type "text/plain": ingest requires one of …`; conflicting variant: `conflicting Content-Type headers: application/json, application/x-ndjson`) | The request declared no `Content-Type`, an unsupported one, or two `Content-Type` declarations that disagree — different formats, or one supported and one not. Checked before the body is parsed |
