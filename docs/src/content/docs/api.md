...
| 503 | `{"error":"dedupe store unavailable"}` | Dedupe is on and its store cannot answer now (not open, or a remote backend throttled or unreachable); `Retry-After: 5`. Nothing was published, so the retry is safe |
