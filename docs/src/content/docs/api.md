...
The header is parsed with Go's `mime.ParseMediaType`, which implements RFC 9110 §8.3 `media-type`, and only the media type decides the format. Parameters are ignored, so one the parser rejects — `application/json; charset`, `application/json;;`, a value left mid-quote — still leaves a readable media type and the request stands. Two exceptions. A parameter name repeated with two *different* values leaves no media type at all, so there is nothing to accept (`; charset=utf-8; charset=utf-16` — an exact repeat is fine). And a **malformed parameter** on a line that also contains a comma is refused *even though the media type reads fine*: the comma may be a second declaration joined on, and the error cannot distinguish that from a comma inside data, so the request is refused rather than guessed at ([#563](https://github.com/Wave-RF/WaveHouse/issues/563)). So `application/json; profile="a,b"` is one media type and is accepted, while `application/json; profile="a,b"; charset` is a `415` — each half alone is fine.

`Content-Type` is also a **singleton** field, and §5.3 forbids repeating it. So anything that isn't exactly one readable media type is a `415`: no header, an unsupported type, one that doesn't parse, or more than one declaration. The single accommodation is for intermediaries that duplicate the header — **repeated header lines** are all resolved and accepted when they agree on the format. A **comma-joined** value is not, because §8.3 warns that picking a member of the resulting pseudo-list is itself an interoperability and security hazard.
...
| 415 | `{"error":"no Content-Type: ingest requires one of application/json, application/x-ndjson, …"}` (declared variant: `Content-Type "text/plain": ingest requires one of …`, naming every header line when several were sent; conflicting variant: `conflicting Content-Type declarations "application/json", "application/x-ndjson": ingest reads one format per request, and requires one of …`) | The request declared no `Content-Type`, one that is unsupported or does not parse (a comma-joined value included), or repeated header lines that disagree — different formats, or one supported and one not. Checked before the body is parsed |
