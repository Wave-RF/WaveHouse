...
| 500 | `{"error":"publish failed"}` | Message queue error whose outcome is unknown: the event may have been stored. With dedupe on, the record's id is left to lapse with the dedupe lease (30 seconds) rather than given back: a retry inside the lease answers the in-flight `503`, and one after it is published under the same idempotency key, which the queue drops if the first copy was stored. The queue remembers the key for its duplicate window after the first publish — two minutes on the embedded broker, the partition stream's `duplicate_window` under [`mq.backend: nats`](/deployment#external-nats) — so a retry inside that window stores no second copy (the SDK's, after the 30-second `Retry-After`, lands inside the embedded one); a later one is stored again. |
...
| 503 | `{"error":"service unavailable"}` | The message queue could not be reached or did not answer in time, mid-batch; includes `Retry-After: 5`. Only under [`mq.backend: nats`](/deployment#external-nats); the embedded broker never reports this, and its publish failures are the `500` above. The records before the failing one keep their ids; its id is left to lapse, as for the `500`, and the rest of its window's are given back |
