...
| 502 | `{"error":"<message>","code":"clickhouse.unknown","retryable":true}` | An answer with no ClickHouse exception code that is not an outage — a `500` from something in front of ClickHouse. A codeless redirect or `4xx` (a wrong path) is `502 clickhouse.misconfigured`, `retryable: false` |
