...
The header is parsed with Go's `mime.ParseMediaType`, which implements RFC 9110 §8.3 `media-type`, and only the media type decides the format. Parameters are ignored, so one the grammar rejects — `application/json; charset`, `application/json;;`, a value left mid-quote — still leaves a readable media type and the request stands. Two exceptions, because they leave no readable media type: a parameter name repeated with two *different* values (`; charset=utf-8; charset=utf-16` — an exact repeat is fine), and **any** parse failure on a line that also contains a comma, which is refused rather than guessed at. So `application/json; profile="a,b"` is one media type and is accepted, while the same value with its closing quote missing is a `415`.
