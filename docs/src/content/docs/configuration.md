...
**TLS + direct-to-cloud OTLP.** `otel.addr` is scheme-aware: `https://` selects TLS (system root CAs), while `http://` or a bare `host:port` stays plaintext gRPC (the default, backward-compatible with prior releases). Combined with `otel.headers` for per-RPC auth, WaveHouse ships telemetry straight to TLS-protected cloud gateways (Grafana Cloud's OTLP gateway, Honeycomb, etc.) — no sidecar required. A sidecar is still useful for egress queuing, batching, and tail-based sampling; it's just no longer mandatory. Datadog has no public direct-to-cloud OTLP endpoint — use the local DDOT Collector path in the [deployment guide](./deployment.md#observability). mTLS / client-certificate auth is not yet supported. See [Deployment → Observability](./deployment.md#observability) for worked Honeycomb / Grafana Cloud examples.
