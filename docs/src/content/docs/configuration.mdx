...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `query.default_max_rows` | `WH_QUERY_DEFAULT_MAX_ROWS` | `10000` | Fallback result `LIMIT` for a structured query when the caller and policy specify none. `0` falls back to the built-in default. |
