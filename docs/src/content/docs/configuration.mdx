...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `stream.heartbeat_interval` | `WH_STREAM_HEARTBEAT_INTERVAL` | `5s` | How often the shared heartbeat ticker fires, pushing a `: heartbeat` keepalive comment to one bucket of live `GET /v1/stream` connections. |
| `stream.heartbeat_buckets` | `WH_STREAM_HEARTBEAT_BUCKETS` | `3` | How many buckets the ticker spreads connections across, so each tick wakes only ~1/N of live streams instead of all at once. |

The **effective per-connection heartbeat period is `heartbeat_interval × heartbeat_buckets`** (default 5s × 3 = 15s) — a connection is pinged once per full rotation of the ring. Keep that product comfortably under your reverse proxy or tunnel's idle timeout (nginx's default `proxy_read_timeout` is 60s; Cloudflare's edge is ~100s) so quiet streams aren't dropped — see [Behind a reverse proxy → Server-Sent Events](/reverse-proxy#server-sent-events-sse). Each tick wakes only one bucket, so more buckets spread the write load across more ticks **but also lengthen the period** (it's the product above): raising `heartbeat_buckets` alone pushes the period out, so lower `heartbeat_interval` proportionally if you want more buckets at the same period. A value of `0` falls back to the default; a negative value is rejected at startup.
...
stream:
  heartbeat_interval: 5s   # effective keepalive period = interval × buckets (default 15s)
  heartbeat_buckets: 3     # keep interval × buckets under your proxy's idle window
...
WH_STREAM_HEARTBEAT_INTERVAL=5s
WH_STREAM_HEARTBEAT_BUCKETS=3
