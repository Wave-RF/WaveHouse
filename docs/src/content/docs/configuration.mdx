...
| `dedupe.reserve_concurrency` | `WH_DEDUPE_RESERVE_CONCURRENCY` | `64` | The most parallel calls one Reserve, Commit or Release makes to a remote dedupe backend, and the idle connections per host the DynamoDB client keeps to match. Ingest sends one id per call today, so the fan-out has no effect yet; `pebble` ignores it. `0` refuses boot. |
