...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `cache.redis.addrs` | `WH_CACHE_REDIS_ADDRS` | *(empty)* | **Required** with `backend: redis`. `host:port` of the server; several are a cluster's seed nodes or the sentinels. Comma-separated in the env var. |
