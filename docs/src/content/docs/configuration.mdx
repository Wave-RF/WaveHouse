...
| `server.shutdown_timeout` | `WH_SERVER_SHUTDOWN_TIMEOUT` | `10` | Seconds each phase of a stop may take: first the drain of in-flight requests and ingest batches (open SSE streams end immediately), then the release of the stores and the telemetry flush. A stop therefore takes at most twice this; see [Stopping](/deployment#stopping). |
