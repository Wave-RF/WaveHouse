...
4. The YAML file is **strict**: a key this page doesn't list — a typo, or a tunable that has moved to the settings directory (`dlq.enabled`, `clickhouse.addr`, `stream.*`, …) — refuses to boot and names every offending key, so nothing is read, ignored, and believed.
...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `mq.max_bytes_gb` | `WH_MQ_MAX_BYTES_GB` | `50` | Maximum NATS JetStream stream size in GB. When full, new publishes are rejected with `DiscardNew` policy, triggering 503 backpressure on the ingest endpoint. The SSE gap window the sweeper keeps inside that budget is `stream.gap_window_minutes` in the [Settings Directory](/settings-directory#streaming). |
...
settings:
  dir: ./settings        # REQUIRED — no default: settings directory (roles/
                         ...
                         # `wavehouse bootstrap ./settings` (dedupe, dlq,
                         # query, schema, stream, cors keys live there)
...
# localhost:9000). The container images preset WH_SETTINGS_DIR=/app/settings
