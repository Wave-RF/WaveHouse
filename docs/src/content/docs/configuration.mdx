...
Everything else — ports, `data_dir`, ClickHouse connection, cache and NATS sizing, auth, `dedupe.enabled` (it owns the Pebble store lifecycle), and the rest — is **restart-only**: a changed value is *reported* (in the reload log line and the endpoint's `restart_required` list) but deliberately not applied, so an edit never half-applies silently. The list is expected to grow (per-table dedupe overrides land with [#222](https://github.com/Wave-RF/WaveHouse/issues/222); log-level hot reload with [#415](https://github.com/Wave-RF/WaveHouse/issues/415)).
