...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `mq.nats.urls` | `WH_MQ_NATS_URLS` | *(none)* | Required. The servers to dial: a YAML list, or a comma-separated variable. |
...
Boot refuses a `nats` block with no URLs, more than one of `creds_file`, `nkey_seed_file` and `user`, half a certificate pair, a prefix outside the grammar, fewer than one partition, or a timeout that is not positive. Durations take Go syntax (`5s`, `2m`).
