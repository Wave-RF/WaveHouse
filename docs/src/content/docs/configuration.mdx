...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `settings.dir` | `WH_SETTINGS_DIR` | *(empty)* | Path to the settings directory. No default, same reasoning as `policy.file_path`: a baked-in path would turn a missing mount into silent misconfiguration instead of an explicit operator choice. |
