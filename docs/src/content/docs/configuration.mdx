...
Boot config (YAML + env) holds what the platform operator owns: wiring, lifecycle, secrets. The tenant-owned behavioral tunables — dedupe `enabled`/`id_field`/`require_id`, `query.default_max_rows`, `schema.refresh_interval`, CORS origins — live in the hot-reloadable [settings directory](/settings-directory) instead.
...
| `clickhouse.http_port` | `WH_CH_HTTP_PORT` | `8123` | ClickHouse HTTP interface port. Used by the ingest worker (`internal/ingest`) for bulk INSERT and by the raw-SQL proxy (`POST /v1/ops/query`, `internal/api/query.go`) to forward SQL to ClickHouse. Schema discovery uses the native protocol on `addr` instead. |
...
No boot-config keys. `dedupe.enabled`, `dedupe.id_field`, `dedupe.require_id`, and the per-table overrides are all settings-directory keys — see [Settings Directory — Deduplication](/settings-directory#deduplication).
...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `dlq.enabled` | `WH_DLQ_ENABLED` | `true` | Enable the Dead Letter Queue. A failed batch insert is retried row by row; the rows that fail again are published to the `WAVEHOUSE_DLQ` NATS stream instead of blocking retries. |
...
policy:
  file_path: ""          # empty = skip bootstrap (seed via PUT /v1/ops/policy);
                         # set to a path and the file MUST exist, parse, and
                         # pass policy validation or boot fails
...
settings:
  dir: ./settings        # REQUIRED — no default: settings directory (roles/
                         ...
                         # `wavehouse init-settings ./settings`
...
WH_SETTINGS_DIR=./settings  # required, no default (`wavehouse init-settings ./settings`); the container images preset /app/settings and mount it
