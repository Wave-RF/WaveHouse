...
Each layer's implementation is chosen once, at boot. Every layer's default is its in-process backend, so a config that sets none of these keys runs as it always has. The message queue also has a shared backend, `nats`; every other layer has only its in-process one so far. A value this build has no backend for refuses boot and names the valid ones.
