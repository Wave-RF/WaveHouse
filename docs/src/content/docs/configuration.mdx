...
- **File change** — the directory is watched; a burst of writes (an editor save, a Kubernetes ConfigMap republish) coalesces into one reload.
- **`SIGHUP`** — `kill -HUP <pid>`.
...
settings:
  dir: ""                # REQUIRED: settings directory (roles/policies/pipes/
                         ...
                         # create one with `wavehouse init-settings <dir>`
