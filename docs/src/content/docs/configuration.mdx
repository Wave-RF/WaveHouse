...
4. If the file exists but cannot be read (permissions, a directory at the path), startup fails with `read config: …` — WaveHouse never silently falls back to environment-only loading.
...
Configuration splits into two disjoint tiers, and every knob lives in exactly one:

- **Boot config** — everything in the [reference below](#full-reference): the YAML file plus `WH_*` environment variables, read once at startup. Changing it means restarting the process; nothing re-reads the file while the server runs.
- **Runtime settings** — live-tunable values stored in WaveHouse's control-plane database (`<data_dir>/control.db`, SQLite — alongside policy and pipes) and changed through the admin API. Updates take effect immediately — no restart, no signal.

Today's runtime settings are the dedupe tunables (per-table dedupe overrides join with [#222](https://github.com/Wave-RF/WaveHouse/issues/222); log level with [#415](https://github.com/Wave-RF/WaveHouse/issues/415)):

| Setting | Default | Effect |
| ------- | ------- | ------ |
| `dedupe.id_field` | `event_id` | JSON field in the ingest body used as the dedup key; subsequent records dedupe on the new field. `""` pauses dedup lookups. |
| `dedupe.require_id` | `false` | A row missing `id_field` (which can't be deduped) is always logged at `WARN` and counted by `wavehouse_ingest_dedupe_missing_id_total`; by default it is then published un-deduped. `true` rejects it instead (`400` for a single insert; a per-record failure in a batch) — a server-side tripwire for producers that must guarantee the id. |

Authenticated as the admin role (or the operator key):

```bash
# Effective values, plus where each section comes from (stored override vs. compiled default):
curl -H "Authorization: Bearer $ADMIN_TOKEN" http://localhost:8080/v1/admin/settings

# Update the dedupe section (full-section PUT — send every field you want set):
curl -X PUT -H "Authorization: Bearer $ADMIN_TOKEN" \
  -d '{"id_field": "event_id", "require_id": true}' \
  http://localhost:8080/v1/admin/settings/dedupe

# Reset the section to its compiled defaults:
curl -X DELETE -H "Authorization: Bearer $ADMIN_TOKEN" http://localhost:8080/v1/admin/settings/dedupe
```

The semantics worth knowing:

- **Defaults are the fallback, and they're safe.** A deployment that never touches the settings API runs on the compiled defaults above, and deleting a section's override reverts to them — there is no fail-closed state, unlike a deleted policy.
- **A bad write can't take the server down.** The API validates before storing (an invalid document is a `400` and changes nothing), and each update applies atomically at a request boundary — requests never see a half-applied section.
- **The environment can't pin a runtime setting.** Runtime settings have no `WH_*` or YAML form at all, so an env-set value can never shadow a live update. The retired keys — `dedupe.id_field` / `WH_DEDUPE_ID_FIELD` and `dedupe.require_id` / `WH_DEDUPE_REQUIRE_ID` — now **refuse boot** with a pointer here, so an upgraded deployment can't silently lose the values it had set.
- **Concurrent admins can't silently overwrite each other.** Different sections are separate tables with independent revisions and never interact. Within a section, send the revision you read as an `If-Match` header on the PUT (`0` for a section still on defaults): a write that lost a race returns `409 Conflict` and changes nothing — re-read and retry on the fresh values, and both edits survive.

Endpoint details (request/response shapes, error contract) are in the [API Reference](/api).
...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `dedupe.enabled` | `WH_DEDUPE_ENABLED` | `false` | Enable event deduplication (restart-only — it owns the Pebble store lifecycle). The dedup key field and strict missing-id handling are [runtime settings](#runtime-settings), not config keys. |
...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `policy.file_path` | `WH_POLICY_FILE_PATH` | *(empty)* | Optional path to a YAML/JSON policy file used to seed the policy store on first startup (when the control-plane database holds no policy). **When set, the file MUST exist and parse — WaveHouse refuses to boot otherwise**, so a typo or missing mount surfaces immediately instead of silently denying every request (`Evaluate` fails closed on a `nil` policy, including the admin role). Empty default — no implicit `policy.yaml` lookup — so operators opt into the bootstrap file explicitly; without one, seed the policy via `PUT /v1/admin/policy`. Once the database is populated, the file is ignored on subsequent boots (the database is the source of truth; runtime updates flow through the API). |

### Named Pipes

| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `pipes.dir` | `WH_PIPES_DIR` | *(empty)* | Optional bootstrap source for named query pipes. When set, `.sql` files in this directory are loaded into the control-plane pipe store on startup. The directory is **read-only at runtime** — it's a seed, not authoritative storage. After bootstrap, the API and the database are the source of truth (runtime pipe edits go through the API, not the files). Empty default skips bootstrap entirely. Mount read-only in containers (e.g. `./my-pipes:/app/pipes:ro`). |
...
<TabItem label="YAML">
```yaml
data_dir: ./data         # control db → ./data/control.db, nats → ./data/nats, pebble → ./data/pebble
