...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `coord.nats.bucket` | `WH_COORD_NATS_BUCKET` | `<prefix>_coord` | The KV bucket the leases live in, one key per lease (`lease.sweeper`). The default follows `mq.nats.subject_prefix` (`wh_coord` for `wh`), as `wavehouse mq manifests` names it, so two deployments sharing one NATS account under different prefixes never contend for one lease. A name outside `[a-zA-Z0-9_-]` refuses boot. |
