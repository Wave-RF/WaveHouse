...
| YAML Key | Env Var | Default | Description |
| --- | --- | ------- | ----------- |
| `stream.heartbeat_interval` | `WH_STREAM_HEARTBEAT_INTERVAL` | `5s` | How often the shared heartbeat ticker fires, pushing a `: heartbeat` keepalive comment to one bucket of idle `GET /v1/stream` connections. |
| `stream.heartbeat_buckets` | `WH_STREAM_HEARTBEAT_BUCKETS` | `3` | How many buckets the ticker spreads connections across, so each tick wakes only ~1/N of idle streams instead of all at once. |
