...
Everything else — ports, `data_dir`, ClickHouse connection, cache and NATS sizing, auth, `dedupe.enabled` (it owns the Pebble store lifecycle), and the rest — is **restart-only**: the first reload after such an edit *reports* the change (in the reload log line and the endpoint's `restart_required` list) but deliberately does not apply it, so an edit never half-applies silently. Each reload reports only what changed since the previous one, so an empty report means no new edits — not that the file matches the running process, which keeps boot values for restart-only keys until it restarts. The list of hot-reloadable fields is expected to grow (per-table dedupe overrides land with [#222](https://github.com/Wave-RF/WaveHouse/issues/222); log-level hot reload with [#415](https://github.com/Wave-RF/WaveHouse/issues/415)).
