...
| `clickhouse.headers` | `{}` | Extra request headers for the HTTP interface (ingest `INSERT`s, `POST /v1/ops/query`), for a proxy or gateway in front of ClickHouse. WaveHouse sets its own `Content-Type`, `X-ClickHouse-User` and `X-ClickHouse-Key` after them, so those win; naming `X-ClickHouse-User`, `X-ClickHouse-Key` or `Authorization` is a validation error. |
| `clickhouse.max_open_conns` | `10` | Native connection pool size (`>= max_idle_conns`). Must also fit under the boot config's [`clickhouse.max_total_conns`](/configuration#clickhouse) when one is set. |
...
**Headers.** `clickhouse.headers` rides on every HTTP-interface request, for a proxy or gateway in front of ClickHouse that wants a token of its own. WaveHouse's own headers — `Content-Type` and the `X-ClickHouse-User` / `X-ClickHouse-Key` credentials — are set after them and win, and naming a credential header is a validation error. The native protocol carries no headers.
