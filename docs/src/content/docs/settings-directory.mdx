...
| `clickhouse.max_open_conns` | `10` | Native connection pool size (`>= max_idle_conns`). Must also not exceed the boot config's [`clickhouse.max_total_conns`](/configuration#clickhouse) when one is set. |
...
- `true` — the row is published to the `WAVEHOUSE_DLQ` NATS stream under `dlq.<tenant>.<table>` with the ClickHouse error in its headers, and its original is acked. Inspect it with `GET /v1/ops/dlq/stats` (admin-only).
