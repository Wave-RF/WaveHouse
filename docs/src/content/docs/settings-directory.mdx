...
| `clickhouse.headers` | `{}` | Extra request headers for the HTTP interface (ingest `INSERT`s, `POST /v1/ops/query`): routing or identification metadata for a proxy or gateway in front of ClickHouse. Values are stored in the file as written, so it is not the place for a secret. WaveHouse sets its own `Content-Type`, `X-ClickHouse-User` and `X-ClickHouse-Key` after them, so those win; naming `X-ClickHouse-User`, `X-ClickHouse-Key` or `Authorization` is a validation error. |
