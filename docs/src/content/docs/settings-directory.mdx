...
For a tenant no longer served — its folder removed or rejected — there is no switch to read: its rows are always parked, so they never hold the ack floor the sweeper purges below.
