...
- Priority on the **Task Board** project #7 is set by hand on the board's `Priority` field. The `PROJECT_BOARD_TOKEN` secret and `.github/board-config.env` existed only for that workflow and are gone with it.
