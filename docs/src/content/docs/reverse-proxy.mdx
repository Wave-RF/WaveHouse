...
- **nginx** — `proxy_buffering off;` on the stream location. (WaveHouse does not set the `X-Accel-Buffering: no` header itself, so configure `proxy_buffering off` explicitly.)
...
- **`X-Forwarded-For` / `X-Forwarded-Proto` / `Host`** — set these so logs and the real-client-IP middleware see the originating client, not the proxy.

:::caution[Don't expose `:8080` directly — `X-Forwarded-For` is trusted]
WaveHouse honors `True-Client-IP`, `X-Real-IP`, and `X-Forwarded-For` (in that precedence — `True-Client-IP` wins) to recover the client IP. That is correct **only** behind a trusted proxy that overwrites those headers. If `:8080` is reachable directly from untrusted clients, a caller can forge any of them — a forged `True-Client-IP` overrides the other two — and spoof their source IP in your logs and any per-IP logic upstream. Bind WaveHouse to a private interface or firewall `:8080` so the proxy is the only path in, and have the proxy set/overwrite all three (not just `X-Forwarded-For`).
:::
...
- **`/livez`** — liveness; sticky-200 after first successful boot.
- **`/readyz`** — readiness; pings ClickHouse on every call. **Point your load balancer's health check here** so it routes around an instance whose ClickHouse is unreachable.
- **`/healthz`** — permanent alias of `/livez`.

These expose nothing sensitive (a failing `/readyz` surfaces at most a ClickHouse connection-error string), so restricting `/livez`/`/readyz`/`/healthz` to internal callers is optional and, if you want it, a proxy concern. **Do not filter `/v1/health`** — it's the SDK's content-free liveness ping and is meant to stay publicly reachable.
