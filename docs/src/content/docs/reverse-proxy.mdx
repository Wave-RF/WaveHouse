...
`GET /v1/stream` is a long-lived `text/event-stream` connection (see the [API reference](/api#get-v1stream--server-sent-events-stream)). Proxies classically break SSE two ways — response buffering, which you must disable, and idle timeouts, which WaveHouse's keepalive heartbeat now handles for you.
...
### 2. Keepalive heartbeats and idle timeouts

WaveHouse sends a periodic SSE comment heartbeat — `: heartbeat` — on every stream (alongside the `: connected` comment on open), so even an otherwise-quiet connection keeps producing bytes between events and an intermediary's idle timeout never trips on a slow table. By default each connection sees a heartbeat about every 15 seconds, comfortably under common proxy and tunnel idle timeouts (nginx's default `proxy_read_timeout` is 60s; Cloudflare's edge is ~100s). A single shared ticker drives keepalives for every connection; the period is configurable via [`stream.heartbeat_interval` × `stream.heartbeat_buckets`](/configuration#streaming-sse) (default 5s × 3 = 15s) — keep that product under your proxy's idle window. The comment is invisible to clients — `EventSource` and any spec-compliant parser ignore SSE comment lines, and `curl` simply prints them.
...
Raising the proxy's read/idle timeout for `/v1/stream` is therefore no longer required, though it's harmless if you'd rather not lean on the heartbeat's margin:
...
- **Cloudflare** — the edge idle timeout isn't configurable on standard plans; the heartbeat is what keeps a quiet stream alive.
...
server {
    ...
    # optional — WaveHouse's keepalive heartbeat (~15s default, #226) keeps quiet streams
...
- The edge resets idle connections after ~100s, but WaveHouse's keepalive heartbeat (~15s by default, [#226](https://github.com/Wave-RF/WaveHouse/issues/226)) keeps even a quiet SSE stream alive, so they're no longer dropped — no reconnect loop needed just to survive a slow table.
