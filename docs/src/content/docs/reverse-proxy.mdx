...
WaveHouse sends a periodic SSE comment heartbeat — `: heartbeat`, every 15 seconds — on an otherwise-quiet stream (alongside the `: connected` comment on open), so the connection keeps producing bytes between events and an intermediary's idle timeout never trips on a slow table. Fifteen seconds sits comfortably under common proxy and tunnel idle timeouts (nginx's default `proxy_read_timeout` is 60s; Cloudflare's edge is ~100s). The timer resets on every delivered event, so a busy stream never sends a redundant heartbeat, and the comment is invisible to clients — `EventSource` and any spec-compliant parser ignore SSE comment lines, and `curl` simply prints them.
...
server {
    ...
    # optional — WaveHouse's 15s keepalive heartbeat (#226) keeps quiet streams
...
- The edge resets idle connections after ~100s, but WaveHouse's 15-second keepalive heartbeat ([#226](https://github.com/Wave-RF/WaveHouse/issues/226)) keeps even a quiet SSE stream alive, so they're no longer dropped — no reconnect loop needed just to survive a slow table.
