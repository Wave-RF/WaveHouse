...
Those cached results are also version-invalidated: when WaveHouse ingests into any base table the pipe reads — resolved through views and materialized views at the time the pipe is created or updated — the next call re-runs the query (`X-Cache: MISS`). Pipes whose dependencies can't be resolved (no schema registry wired, or SQL the resolver can't expand) fall back to plain TTL expiry.
