...
<CardGrid>
 ...
 <Card title="TypeScript & Go SDKs" icon="seti:typescript">
  `@wavehouse/sdk` and `github.com/Wave-RF/WaveHouse/clients/go` — type-safe query builders, live queries, real-time streaming, and codegen from your schemas, speaking one shared wire format.
 </Card>
</CardGrid>
...
The [client SDKs](/sdk) wrap the whole surface — typed inserts, a chainable query builder, and live queries that backfill history before streaming. Examples below are TypeScript; the Go client mirrors the same feature set, and every SDK page carries both languages side by side.
...
<CardGrid>
 ...
 <LinkCard
  title="Client SDKs"
  description="TypeScript and Go — query builder, live queries, streaming, and schema codegen."
  href="/sdk"
 />
</CardGrid>
