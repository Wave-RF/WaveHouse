...
| `backoff.go` | The retry backoff per ClickHouse pool: one outage backs off every table on it together, probing once per window — see [When ClickHouse cannot take an insert](#when-clickhouse-cannot-take-an-insert) |
...
| `Denied` | `AUTHENTICATION_FAILED`, `ACCESS_DENIED`, `UNKNOWN_USER`, `WRONG_PASSWORD`, `IP_ADDRESS_NOT_ALLOWED`, `DATABASE_ACCESS_DENIED`, `USER_EXPIRED`; a `401`/`403` with no code | Retry with backoff — the credentials or grants are wrong, not the rows |
