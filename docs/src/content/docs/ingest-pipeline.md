...
| `Unavailable` | Connection refused/reset, DNS, timeouts (the client's and `TIMEOUT_EXCEEDED`/`SOCKET_TIMEOUT`), `NETWORK_ERROR`, `TOO_MANY_SIMULTANEOUS_QUERIES`, `MEMORY_LIMIT_EXCEEDED`, `TOO_MANY_PARTS`, `READONLY`, `TABLE_IS_READ_ONLY`, `NOT_ENOUGH_SPACE`, `KEEPER_EXCEPTION`, `ALL_CONNECTION_TRIES_FAILED`, lost replicas and quorum, `UNKNOWN_STATUS_OF_INSERT`, …; a `502`/`503`/`504`/`429`/`408` with no exception code | Retry with backoff; never isolated, never dead-lettered |
