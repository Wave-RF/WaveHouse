...
// inserting CanonicalizeTimestamps' output must (a) both succeed or both
// fail, and (b) when both succeed, store the same instant. A same-instant
// failure means canonicalization rewrote a value ClickHouse reads differently
// (data corruption); an asymmetric accept means canonicalization changed
// which values are insertable (ClickHouse, not the canonicalizer, is the
// arbiter of insertability).
//
// Inserts go through the same HTTP surface the ingest worker uses —
// JSONEachRow with date_time_input_format=best_effort — so the oracle is the
// production parse, not a lookalike.
func TestTimestampCanonicalization_DifferentialAgainstClickHouse(t *testing.T) {
	...
	// One entry per spelling family. The calendar-shaped digit runs and the
	// garbage entries document the pass-through side of the contract: they
	// must reach ClickHouse verbatim and get its verdict, never a rewrite.
	corpus := []any{
