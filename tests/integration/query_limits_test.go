...
func TestStructuredQuery_ResourceCapsEnforcedServerSide(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			name:        "max_rows_to_read is enforced (code 158 TOO_MANY_ROWS)",
		...
		},
		{
			...
			name:        "max_memory_usage_bytes is enforced (code 241 MEMORY_LIMIT_EXCEEDED)",
			perms:       policy.RolePermissions{AllowColumns: []string{"*"}, MaxMemoryUsageBytes: 1},
			wantStatus:  http.StatusInternalServerError,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			h := api.NewStructuredQueryHandler(
				e.chConn, nil, e.registry, store, 60, 30*time.Second, testutil.NopLogger(),
			)
