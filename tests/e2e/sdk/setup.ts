...
// TODO: This is a terrible way to do this, need to fix
...
function makeSetupJWT(): string {
  ...
  const sig = createHmac("sha256", "sdk-dev-secret")
    .update(unsigned)
    .digest("base64url");
  return `${unsigned}.${sig}`;
}
...
export async function setup(): Promise<void> {
  ...
  if (state.mode === "dev") {
    ...
    console.log(
      "  All tests will run including JWT validation + policy enforcement.\n",
    );
  }
}
