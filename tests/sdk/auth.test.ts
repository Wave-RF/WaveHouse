...
describe('Auth', () => {
  ...
  it('viewer role can read data', async () => {
    if (isDevMode()) {
      console.log('    ⏭  Skipped: viewer role read (auth not enforced in dev mode)');
      return;
    }
  ...
  it('expired JWT is rejected', async () => {
    if (isDevMode()) {
      console.log('    ⏭  Skipped: expired JWT rejection (auth not enforced in dev mode)');
      return;
    }

    const wh = createClient({
      baseURL: WH_URL,
      auth: () => makeExpiredJWT({ sub: 'expired-user', role: 'viewer', tenant_id: 'acme' }),
    });
  ...
  it('invalid JWT is rejected', async () => {
    if (isDevMode()) {
      console.log('    ⏭  Skipped: invalid JWT rejection (auth not enforced in dev mode)');
      return;
    }
  ...
  it('unauthenticated request is rejected when auth is enabled', async () => {
    if (isDevMode()) {
      console.log('    ⏭  Skipped: unauthenticated rejection (auth not enforced in dev mode)');
      return;
    }
