...
import { dataClient, viewerClient, adminClient, waitForCondition, testId, chQuery, isDevMode } from './helpers.js';
...
describe('Ingest', () => {
  ...
  it('inserts with viewer role', async () => {
    if (isDevMode()) {
      console.log('    ⏭  Skipped: viewer role insert (auth not enforced in dev mode)');
      return;
    }
