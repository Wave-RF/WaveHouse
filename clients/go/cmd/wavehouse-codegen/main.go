...
func chTypeToGo(chType string) string {
	...
	case strings.HasPrefix(chType, "Decimal"),
		strings.HasPrefix(chType, "UInt64"),
		strings.HasPrefix(chType, "UInt128"),
		strings.HasPrefix(chType, "UInt256"),
		strings.HasPrefix(chType, "Int64"),
		strings.HasPrefix(chType, "Int128"),
		strings.HasPrefix(chType, "Int256"):
		// 64-bit and bigger integers (and decimals) are strings on the wire:
		// ClickHouse's JSON output quotes them by default
		// (output_format_json_quote_64bit_integers=1) and the server forwards
		// the ClickHouse `data` array verbatim.
		return "string"
	}
	// Array.
