...
func newCapturingHandler(t *testing.T, conn driver.Conn, p *policy.Policy) *StructuredQueryHandler {
	...
	return NewStructuredQueryHandler(conn, nil, reg, policy.NewMemoryStore(p), func() int { return 60 }, func() time.Duration { return 5 * time.Second }, nil, testutil.NopLogger())
}
