...
func newStructuredQueryHandler() *StructuredQueryHandler {
	...
	return NewStructuredQueryHandler(nil, nil, reg, nil, 60, 5*time.Second, QueryLimits{}, testutil.NopLogger())
}
...
func newCapturingHandler(t *testing.T, conn driver.Conn, p *policy.Policy) *StructuredQueryHandler {
	...
	return NewStructuredQueryHandler(conn, nil, reg, policy.NewMemoryStore(p), 60, 5*time.Second, QueryLimits{}, testutil.NopLogger())
}
