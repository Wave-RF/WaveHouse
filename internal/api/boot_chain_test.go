...
// BootState carries the diagnostic → /health returns 503 with the wrapped
...
func TestBoot_Chain_DegradedThenRecovers(t *testing.T) {
	...
	// Phase 1 — /health surfaces the diagnostic verbatim with 503. This is
	// the operator-facing contract: `curl /health` answers "why can't this
	// process serve traffic" instead of the operator having to grep a
	// restart-loop log.
	rec := httptest.NewRecorder()
	handler.Liveness(rec, httptest.NewRequestWithContext(context.Background(), http.MethodGet, "/health", nil))
	...
	// Phase 3 — /health flips to 200 once BootState is cleared. This is
	...
	// /health remains 200 even if ClickHouse goes unreachable later
	// (that case is reflected in /ready, not /health — covered by the
