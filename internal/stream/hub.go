...
func (h *Hub) ReplayProjector(role string, sub *Subscriber) func(raw []byte) []Frame {
	...
	return func(raw []byte) []Frame {
		...
		if plan.schema != nil && lastSig != plan.sig {
			lastSig = plan.sig
			return []Frame{*plan.schema, plan.data}
		}
...
type rolePlan struct {
	...
	sig    string
	schema *Frame // nil ⇒ nothing to announce (the legacy passthrough)
	data   Frame
}
...
func planForRole(p *policy.Policy, filter bool, role string, ev *eventView, kind string) (rolePlan, bool) {
	...
	schema := Frame{Kind: KindSchema, Data: schemaFrame(ev.evt.TableName, projected)}
	return rolePlan{
		perms:  perms,
		sig:    sig,
		schema: &schema,
