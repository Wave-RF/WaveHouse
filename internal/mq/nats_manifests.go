...
// natsManifestObjects is the topology as nack CRs, in the order an operator
// should apply them: partitions and their durables before the history, whose
// source consumers copy only what is published after they exist.
...
# Apply the partitions and durables before the history: a history source only
# copies rows that are still in the partition when it first attaches.
