...
func TestValidate_ContentRules(t *testing.T) {
	...
	tests := []struct {
		...
		{"clickhouse.headers reserved credential", FileConfig, `{"clickhouse": {"headers": {"x-clickhouse-key": "v"}}}`, "clickhouse.headers.x-clickhouse-key: set by WaveHouse itself"},
		{"clickhouse.headers reserved authorization", FileConfig, `{"clickhouse": {"headers": {"Authorization": "Basic xyz"}}}`, "clickhouse.headers.Authorization: set by WaveHouse itself"},
