...
// byte as %XX (uppercase hex), so no separator, wildcard, whitespace, brace or
// non-ASCII byte ever appears in it unescaped, and any table name ClickHouse
// accepts encodes. The bytes it keeps are exactly a tenant id's
...
// and dedupe stores — so a change to what it keeps orphans them. Earlier
// builds escaped '-' as %2D; Unescape still reads that form.
package keyenc
