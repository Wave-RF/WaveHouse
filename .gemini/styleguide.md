...
- **Broken auth / authz**: JWT claim handling, role extraction (`auth.role_claim`), policy templating (`{{ jwt.path }}`), raw-SQL access from outside the `admin` / `service` role set on `/v1/admin/query`.
