...
- **`claude-review.yml` waits for required CI to settle and skips on red.** New first step polls `gh pr checks --watch` until the PR's required checks (`Check`, `Build`, `Validate`) reach a terminal state, then short-circuits if any of them failed/cancelled/timed-out. Stops burning Claude review tokens on PRs the human will bounce back as "fix CI first." Re-pushing green CI re-triggers the review normally.
